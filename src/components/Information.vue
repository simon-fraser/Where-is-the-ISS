  ...
  <div class="detpanel">
    ...
    <div class="container grid-md">
      <div class="columns">
          ...
          <h3>Current Crew</h3>
            <ul class="crewlist">
              ...
              <li v-for="cx in crew" :key="cx.name">
                <a :href="`https://en.wikipedia.org/wiki/${ cx.name.replace(' ', '_') }`" target="_blank">
                  <figure class="avatar avatar-lg mr-2">
                    <AstroImage :name="cx.name" />
                  </figure>{{ cx.name }}
                </a>
              </li>
        ...
        <div class="column col-md-12 facts">
...
<script>
import AstroImage from './PromiseAstroImage.vue'
  ...
  data () {
    return {
      ...
      facts: [
        ...
        "Pressurized Module Length is 240 feet (73 meters)",
        "Truss Length is 357.5 feet (109 meters)",
        "The Solar Array Length is 239.4 feet (73 meters)",
        "Mass is 925,335 pounds (419,725 kilograms)",
        "Habitable Volume is 13,696 cubic feet (388 cubic meters) not including visiting vehicles",
        "Pressurized Volume is 32,333 cubic feet (916 cubic meters)",
        "Power Generation by 8 solar arrays provide 75 to 90 kilowatts of power",
        "There are approximately 2.3 million lines of Computer Code",
      ],
    }
  },
  methods: {
    getCrew () {
      fetch(process.env.VUE_APP_ISS_CREW)
        .then((response) => {
    ...
    randomFact () {
      const fl = this.facts.length
      const rand = Math.floor(Math.random() * fl)
      return this.facts[rand]
    }
  },
  mounted: function () {
    this.getCrew()
